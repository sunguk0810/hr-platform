...
public class CommonCodeController {
    ...
    @GetMapping
    @Operation(summary = "공통 코드 전체 목록 조회 (페이징)")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<PageResponse<CommonCodeResponse>>> getAllCommonCodes(
            @RequestParam(required = false) String keyword,
            @RequestParam(required = false) String groupCode,
            @RequestParam(required = false) CodeStatus status,
            @PageableDefault(size = 20) Pageable pageable) {
        Page<CommonCodeResponse> page = commonCodeService.getAll(keyword, groupCode, status, pageable);
