...
import com.hrsaas.mdm.domain.event.CodeGroupUpdatedEvent;
...
public class CodeGroupServiceImpl implements CodeGroupService {
    ...
    public CodeGroupResponse create(CreateCodeGroupRequest request) {
        ...
        // G01: tenantId가 null이면 시스템 코드 → SUPER_ADMIN 필요
        if (tenantId == null) {
    ...
    public CodeGroupResponse update(UUID id, UpdateCodeGroupRequest request) {
        ...
        codeGroup.update(
            ...
            request.getGroupNameEn(),
            request.getDescription(),
            request.getHierarchical(),
            request.getMaxLevel(),
        ...
        // Publish event
        eventPublisher.publish(CodeGroupUpdatedEvent.of(saved));

        log.info("Code group updated: id={}, groupCode={}", id, saved.getGroupCode());
    ...
    public void delete(UUID id) {
        ...
        if (codeGroup.isSystemCode()) {
            // G01: 시스템 코드 그룹 삭제 시 SUPER_ADMIN 필요
            permissionChecker.requireRole("ROLE_SUPER_ADMIN");
