...
if [ "$LOAD_SAMPLE" = true ]; then
    ...
    if [ -f "$SAMPLE_DATA_DIR/99_combined_all.sql" ]; then
        ...
        echo "[WARNING] Sample data file not found: $SAMPLE_DATA_DIR/99_combined_all.sql"
        echo "Run: cd $SAMPLE_DATA_DIR && python combine_sql.py"
        exit 1
    fi
fi
...
echo "  psql -h localhost -p 5433 -U hr_saas -d hr_saas"
echo ""
echo "View tables:"
